...
static MODULE(evilunit_contract_explicit_libc)
{
  enum {is_none = 0};
  CHECK(depth() == 0);
  ...
  {
    int x = 0;
    if (EVILUNIT_CONTRACT_LIBC_SETJUMP_IMPL() == 0)
      {
        CHECK(depth() == 1);
        x = 1;
      }
...
static MODULE(evilunit_contract_default_libc)
{
  enum {is_none = 0};
  CHECK(depth() == 0);
...
static MODULE(evilunit_contract_cross_libc)
{
  enum {is_none = 0};
  CHECK(depth() == 0);
...
static MODULE(evilunit_contract_death_libc)
{
  CHECK(depth() == 0);
...
MODULE(evilunit_contract_libc)
{
  DEPENDS(evilunit_contract_explicit_libc);
  DEPENDS(evilunit_contract_default_libc);
  DEPENDS(evilunit_contract_cross_libc);
  DEPENDS(evilunit_contract_death_libc);
}
