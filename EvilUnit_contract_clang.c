...
#define EVILUNIT_CONTRACT_DEFAULT_IMPLEMENTATION EVILUNIT_CONTRACT_IMPLEMENTATION_CLANG
...
static MODULE(evilunit_contract_explicit_clang)
{
  enum {is_none = 0};
  CHECK(depth() == 0);
  ...
  {
    int x = 0;
    if (EVILUNIT_CONTRACT_CLANG_SETJUMP_IMPL() == 0)
      {
        CHECK(depth() == 1);
        x = 1;
      }
...
static MODULE(evilunit_contract_default_clang)
{
  enum {is_none = 0};
  CHECK(depth() == 0);
...
static MODULE(evilunit_contract_cross_clang)
{
  enum {is_none = 0};
  CHECK(depth() == 0);
...
static MODULE(evilunit_contract_death_clang)
{
  CHECK(depth() == 0);
...
MODULE(evilunit_contract_clang)
{
  DEPENDS(evilunit_contract_explicit_clang);
  DEPENDS(evilunit_contract_default_clang);
  DEPENDS(evilunit_contract_cross_clang);
  DEPENDS(evilunit_contract_death_clang);
}
